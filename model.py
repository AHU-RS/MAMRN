...
class DMCN_prelu(nn.Module):
    def __init__(self, BN=True, width=64):
        ...
        self.down_sample1 = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, stride=2, padding=1,
                                      bias=False)
        self.Conv_DW_layers1 = self.make_layer(DwSample, 5, BN, width)

        self.down_sample2 = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, stride=2, padding=1,
                                      bias=False)
        self.Conv_DW_layers2 = self.make_layer(DwSample, 2, BN, width)

        self.up_sample1 = UpSample(width, 2)

        self.choose1 = nn.Conv2d(in_channels=width * 2, out_channels=width, kernel_size=1, stride=1, padding=0,
                                 bias=False)
        self.resudial_layers1 = self.make_layer(BasicBlock, 2, BN, width)

        self.up_sample2 = UpSample(width, 2)

        self.choose2 = nn.Conv2d(in_channels=width * 2, out_channels=width, kernel_size=1, stride=1, padding=0,
                                 bias=False)
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, res_down=True):
...
class MAMRNet(nn.Module):
    def __init__(self, n_channels=1, n_classes=1, res_down=True, n_resblocks=1, padding_type="reflect",
                 norm_layer=nn.BatchNorm2d, use_dropout=False, use_bias=True, bilinear=True):
        super(MAMRNet, self).__init__()
    ...
    def forward(self, x):
        x1 = self.inc(x)  # 64
        sa1_output = self.sa1(x1)

        x2 = self.down1(x1)  # 128
        sa2_output = self.sa2(x2)

        x3 = self.down2(x2)  # 256
        sa3_output = self.sa3(x3)

        x4 = self.down3(x3)  # 512
        sa4_output = self.sa4(x4)
        ...
        xp1 = self.up1(u5, u4)  # 512
        xp2 = self.up2(xp1, u3)  # 256
        xp3 = self.up3(xp2, u2)  # 128
        xp4 = self.up4(xp3, u1)  # 64
        xp5 = self.pa2(xp4)+xp4

        logits = self.outc(xp5) + x
        return logits